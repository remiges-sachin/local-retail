...
start_caddy_logs() {
  if [[ ${EUID} -eq 0 ]]; then
    start_prefixed_stream caddy journalctl -u caddy -f -n "${TAIL_LINES}" --no-pager
    return
  fi

  if command -v sudo >/dev/null 2>&1 && sudo -n true >/dev/null 2>&1; then
    start_prefixed_stream caddy sudo -n journalctl -u caddy -f -n "${TAIL_LINES}" --no-pager
    return
  fi
...
start_onix_logs() {
  (
    cd "${ONIX_COMPOSE_DIR}"
    stdbuf -oL -eL docker compose -f "${ONIX_COMPOSE_FILE}" logs -f --tail "${TAIL_LINES}" --no-color onix-bap onix-bpp 2>&1 \
      | sed -u \
...
start_app_logs() {
  (
    cd "${APP_COMPOSE_DIR}"
    stdbuf -oL -eL docker compose -f "${APP_COMPOSE_FILE}" logs -f --tail "${TAIL_LINES}" --no-color bap bpp frontend 2>&1 \
      | sed -u \
          -e 's/^bap[[:space:]]*|/[bap] /' \
          -e 's/^bpp[[:space:]]*|/[bpp] /' \
...
start_caddy_access_log() {
  if [[ -f "${CADDY_ACCESS_LOG}" ]]; then
    if [[ ${EUID} -eq 0 ]]; then
      start_prefixed_stream caddy-access tail -n "${TAIL_LINES}" -F "${CADDY_ACCESS_LOG}"
      return
    fi

    if command -v sudo >/dev/null 2>&1 && sudo -n true >/dev/null 2>&1; then
      start_prefixed_stream caddy-access sudo -n tail -n "${TAIL_LINES}" -F "${CADDY_ACCESS_LOG}"
      return
    fi
...
start_callback_log() {
  ...
  if [[ -f "${CALLBACK_LOG}" ]]; then
    start_prefixed_stream callback tail -n "${TAIL_LINES}" -F "${CALLBACK_LOG}"
  else
...
echo "[tail-all-logs] Showing latest ${TAIL_LINES} lines per source before follow mode"
if [[ "${INCLUDE_CALLBACK_LOG}" == "1" ]]; then
